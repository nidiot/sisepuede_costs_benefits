#this is the script for teh ALL transformation (#7)
...
for (i in nations){
  ...
  datap_all<-subset(datap,policy==all_policy)[,c(id_vars,ccs_vars)]
  emissions_differences<-datap0[,c(ccs_vars)]-datap_all[,c(ccs_vars)]
  costs<-emissions_differences*ccs_cost_per_tco2
  ccs_output[rowstart:rowend,c(id_vars)]<-datap_all[,c(id_vars)]
...
for (i in nations){
  ...
  datap_all<-subset(datap,policy==all_policy)[,c(id_vars,fgas_vars)]
  emissions_differences<-datap0[,c(fgas_vars)]-datap_all[,c(fgas_vars)]
  costs<-emissions_differences*fgas_cost_per_tco2
  fgas_output[rowstart:rowend,c(id_vars)]<-datap_all[,c(id_vars)]
...
for (i in nations){
  ...
  datap_all<-subset(datap,policy==all_policy)[,c(id_vars,n2o_vars)]
  emissions_differences<-datap0[,c(n2o_vars)]-datap_all[,c(n2o_vars)]
  costs<-emissions_differences*n2o_cost_per_tco2
  n2o_output[rowstart:rowend,c(id_vars)]<-datap_all[,c(id_vars)]
...
for (i in nations){
  ...
  costs<-t(t(materials_differences)*dm_cost_per_tonne)
  benefits_waste<-t(t(materials_differences)*dm_waste_ben_per_ton)
  benefits_mining<-t(t(materials_differences)*dm_mining_ben_per_ton)
  benefits_air<-t(t(materials_differences)*dm_air_ben_per_ton)
...
for (i in nations){
  ...
  benefits<-t(t(datap6[,c(rec_vars)])*rec_ben_per_ton)
...
# METALS (steel) ---------
#steel transformations are easier to do simply by recalculating.
#(1) demand reduction dr
#(2) recycling
#(3) ccs


#(1) all transformaiton: metals demand reduction benefits
all_metals_dm_vars <-c('prod_ippu_metals_tonne')

all_metals_dm_cost_per_tonne<-c(-500)
all_metals_dm_waste_ben_per_ton<-c(-390)
all_metals_dm_mining_ben_per_ton<-c(3)
all_metals_dm_air_ben_per_ton<-c(0)

all_metals_dm_diff_vars<-paste0('dm_avoided_tons_', all_metals_dm_vars)
all_metals_dm_cost_vars<-paste0('dm_cost_USD_', all_metals_dm_vars)
all_metals_dm_ben_vars_avoided_waste<-paste0('dm_benefit_avoided_waste_byproducts_USD_', all_metals_dm_vars)
all_metals_dm_ben_vars_avoided_mining<-paste0('dm_benefit_avoided_mining_USD_', all_metals_dm_vars)
all_metals_dm_ben_vars_avoided_air<-paste0('dm_benefit_avoided_airpollution_USD_', all_metals_dm_vars)

all_metals_dm_output_rownames<-c(id_vars, all_metals_dm_diff_vars, 
                                 all_metals_dm_cost_vars, 
                                 all_metals_dm_ben_vars_avoided_mining,
                                 all_metals_dm_ben_vars_avoided_air)

#EDMUNDO: the dm output will be in this data frame at the end of the for loop
all_metals_dm_output<-data.frame(matrix(ncol=length(all_metals_dm_output_rownames), nrow=numtimesteps*numnations))
colnames(all_metals_dm_output)<- all_metals_dm_output_rownames
...
for (i in nations){
  #datap<-subset(data,nation=='brazil')
  datap<-subset(data,nation==i)
  datap0<-subset(datap,policy==0)[,c(id_vars, all_metals_dm_vars)]
  datap7<-subset(datap,policy==7)[,c(id_vars, all_metals_dm_vars)]
  materials_differences<-datap0[,c(all_metals_dm_vars)]-datap7[,c(all_metals_dm_vars)]
  costs<-t(t(materials_differences)*all_metals_dm_cost_per_tonne)
  benefits_waste<-t(t(materials_differences)*all_metals_dm_waste_ben_per_ton)
  benefits_mining<-t(t(materials_differences)*all_metals_dm_mining_ben_per_ton)
  benefits_air<-t(t(materials_differences)*all_metals_dm_air_ben_per_ton)
  
  all_metals_dm_output[rowstart:rowend,c(id_vars)]<-datap7[,c(id_vars)]
  all_metals_dm_output[rowstart:rowend,c(dm_diff_vars)]<-materials_differences
  all_metals_dm_output[rowstart:rowend,c(dm_cost_vars)]<-costs
  all_metals_dm_output[rowstart:rowend,c(dm_ben_vars_avoided_waste)]<-benefits_waste
  all_metals_dm_output[rowstart:rowend,c(dm_ben_vars_avoided_mining)]<-benefits_mining
  all_metals_dm_output[rowstart:rowend,c(dm_ben_vars_avoided_air)]<-benefits_air
  all_metals_rowstart<-rowend+1
  all_metals_rowend<-rowend+numtimesteps
}

#(2) all transformaiton: recycling benefits
#Cost of rec is in circular economy. Benefit is value of materials, but will be endognized later
all_metals_rec_vars <-c('qty_ippu_recycled_metals_used_in_production_tonne')
all_metals_rec_ben_per_ton<-c(1307)

all_metals_rec_ben_vars<-paste0('recycle_benefit_savings_', all_metals_rec_vars)
all_metals_rec_output_rownames<-c(id_vars, 
                                  all_metals_rec_ben_vars)

#EDMUNDO: the recycling output will be in this data frame at the end of the for loop
all_metals_rec_output<-data.frame(matrix(ncol=length(all_metals_rec_output_rownames), nrow=numtimesteps*numnations))
colnames(all_metals_rec_output)<- all_metals_rec_output_rownames
...
for (i in nations){
  #datap<-subset(data,nation=='brazil')
  datap<-subset(data,nation==i)
  datap7<-subset(datap,policy==7)[,c(id_vars, all_metals_rec_vars)]
  benefits<-t(t(datap6[,c(all_metals_rec_vars)])*all_metals_rec_ben_per_ton)
  
  all_metals_rec_output[rowstart:rowend,c(id_vars)]<-datap7[,c(id_vars)]
  all_metals_rec_output[rowstart:rowend,c(all_metals_rec_ben_vars)]<-benefits
  rowstart<-rowend+1
  rowend<-rowend+numtimesteps
}

#(3) CCS for metals
all_metals_ccs_vars <-c('emission_co2e_co2_ippu_production_metals')
all_metals_ccs_cost_per_tco2<-c(55)
all_metals_ccs_categories<-substring(all_metals_ccs_vars, 35)
all_metals_ccs_cost_vars<-paste0('cost_USD_M_ccs_', all_metals_ccs_categories)
all_metals_ccs_diff_vars<-paste0('difference_ccs_', all_metals_ccs_vars)

all_metals_ccs_output_rownames<-c(id_vars, all_metals_ccs_cost_vars, all_metals_ccs_diff_vars)

#EDMUNDO: the ccs output will be in this data frame at the end of the for loop
all_metals_ccs_output<-data.frame(matrix(ncol=length(all_metals_ccs_cost_output_rownames), nrow=numtimesteps*numnations))
colnames(all_metals_ccs_output)<- all_metals_ccs_output_rownames
...
for (i in nations){
  #datap<-subset(data,nation=='brazil')
  datap<-subset(data,nation==i)
  datap0<-subset(datap,policy==0)[,c(id_vars,all_metals_ccs_vars)]
  datap7<-subset(datap,policy==7)[,c(id_vars,all_metals_ccs_vars)]
  emissions_differences<-datap0[,c(all_metals_ccs_vars)]-datap7[,c(all_metals_ccs_vars)]
  costs<-emissions_differences*all_metals_ccs_cost_per_tco2
  all_metals_ccs_output[rowstart:rowend,c(id_vars)]<-datap7[,c(id_vars)]
  all_metals_ccs_output[rowstart:rowend,c(all_metals_ccs_cost_vars)]<-costs
  all_metals_ccs_output[rowstart:rowend,c(all_metals_ccs_diff_vars)]<-emissions_differences
  rowstart<-rowend+1
  rowend<-rowend+numtimesteps
}
