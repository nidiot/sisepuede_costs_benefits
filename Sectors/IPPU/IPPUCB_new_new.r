#output.file<-read.csv("C:\\Users\\Usuario\\OneDrive\\Edmundo-ITESM\\3.Proyectos\\42. LAC Decarbonization\\Git-LAC-Calib\\lac_decarbonization\\outputs\\output_ippu_all_2022_06_24.csv")
dir_output<-r"(C:\Users\AP03054557\OneDrive\Edmundo-ITESM\3.Proyectos\42. LAC Decarbonization\Git-LAC-Calib\lac_decarbonization\outputs\)"
#dir_output<-r"(C:\Users\Usuario\OneDrive\Edmundo-ITESM\3.Proyectos\42. LAC Decarbonization\Git-LAC-Calib\lac_decarbonization\outputs\)"
#library(reshape)
output.file<-read.csv(paste0(dir_output,"output_ippu_all_2022_11_24.csv"))
data<-output.file
...
ccs_output<-data.frame(matrix(ncol=length(ccs_output_rownames), nrow=numtimesteps*numnations))
...
for (i in nations){
  ...
  datap5<-subset(datap,policy==5)[,c(id_vars,n2o_vars)]
  emissions_differences<-datap0[,c(n2o_vars)]-datap5[,c(n2o_vars)]
...
clinker<-read.csv('C:\\Users\\AP03054557\\OneDrive\\Edmundo-ITESM\\3.Proyectos\\42. LAC Decarbonization\\Git-LAC-Calib\\lac_decarbonization\\calibration\\IPPU\\Transformations\\clinker.csv')
...
for (i in nations){
  ...
  costs<-clinker_substituted_tons*clinker_cost_per_tonne/10^6
  benefits<-clinker_substituted_tons*clinker_ben_per_tonne/10^6
...
for (i in nations){
  ...
  costs_technical<-t(t(materials_differences)*dm_technical_cost_per_tonne)/10^6
  ...
  dm_output[rowstart:rowend,c(dm_cost_vars_technical)]<-costs_technical
...
Reduce(function(...) merge(..., all=T), list(rec_output,
                                                                dm_output#,
                                                                #clinker_output,
                                                                #n2o_output,
                                                                #fgas_output,
...
#merge all
CostBenefits_all<- Reduce(function(...) merge(..., all=T), list(rec_output,
                                                                dm_output,
                                                                clinker_output,
                                                                n2o_output,
                                                                fgas_output,
...
#all in millions
#CostBenefits_all$clinker_substituted_cost_USD_prod_ippu_cement_tonne<-CostBenefits_all$clinker_substituted_cost_USD_prod_ippu_cement_tonne/1e6
#CostBenefits_all$dm_cost_USD_prod_ippu_cement_tonne<-CostBenefits_all$dm_cost_USD_prod_ippu_cement_tonne/1e6
#CostBenefits_all$dm_cost_USD_prod_ippu_metals_tonne<-CostBenefits_all$dm_cost_USD_prod_ippu_metals_tonne/1e6
...
data<-data.table(CostBenefits_all)
DT.m2 = melt(data, id.vars = id_vars,
                   measure.vars = subset(colnames(data),!(colnames(data)%in%id_vars)),
             )


dir_out<-r"(C:\Users\AP03054557\OneDrive\Edmundo-ITESM\3.Proyectos\42. LAC Decarbonization\Git-LAC-Calib\lac_decarbonization\calibration\IPPU\Tableau\new\)"
...
# METALS--------
# (1) demand management costs and benefits are calculated
# (2) recycling benefits are calculated
# (3) ccs costs are reduced by (a) demand management fraction over time x (b) estimated (2.5%) fractional emissions reductions from recycling transformation
# CHEM----
# (1) calculate CCS as difference in CO2 emissions from baseline * cost
# GLASS ---


#FGAS and N2O transformations, just calculate the difference from baseline
#as we did with these individual transformations
